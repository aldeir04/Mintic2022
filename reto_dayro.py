def calcularInforme(creditos: dict) -> list:
    ...
    for usuarioCodigo, valores in creditos.items():
        if valores['est_credito'] != 'Pagado':
            ...
            mensaje = codigo + '-' + nombre + '-' + \
                apellido  # primer elemento de la respuesta
            mensaje2.append(codigo+'-'+nombre + '-' + apellido)
            codigoUsuarios.append(mensaje)  # Añadir a variable codigoUsuario
            print(codigo)
            cuotaBase = valores['credito'][0]['valor'] / \
                valores['credito'][0]['cuotas']  # definir cuota base
            # definir cuota interes
